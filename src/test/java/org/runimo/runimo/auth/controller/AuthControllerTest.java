...
class AuthControllerTest {
  ...
  void 카카오_로그인_INVALID_401응답() throws Exception {
    // given
    given(oidcService.kakaoLogin(any())).willThrow(UserJwtException.of(UserHttpResponseCode.LOGIN_FAIL_INVALID));
    // when & then
    mockMvc.perform(post("/api/v1/auth/kakao")
        ...
        .andExpect(jsonPath("$.message").value(UserHttpResponseCode.LOGIN_FAIL_INVALID.getClientMessage()));
  }
  ...
  void 카카오_로그인_미등록_사용자_404응답() throws Exception {
    ...
    mockMvc.perform(post("/api/v1/auth/kakao")
        ...
        .andExpect(jsonPath("$.error_code").value(UserHttpResponseCode.LOGIN_FAIL_NOT_SIGN_IN.getCode()))
        .andExpect(jsonPath("$.message").value(UserHttpResponseCode.LOGIN_FAIL_NOT_SIGN_IN.getClientMessage()));
  }

  @Test
  void 카카오_로그인_성공_200응답() throws Exception {
    // given
    AuthResponse authResponse = new AuthResponse("test-nickname", "imgurl", "access-token", "refresh-token");
  ...
  void 회원가입_토큰_INVALID_401응답() throws Exception {
    ...
    mockMvc.perform(post("/api/v1/auth/signup")
            .contentType(MediaType.APPLICATION_JSON)
            .content("{\"register_token\":\"invalid-token\", \"nickname\":\"RunimoUser\", \"img_url\":\"https://example.com/image.jpg\"}"))
        .andExpect(status().isUnauthorized())
        .andExpect(jsonPath("$.code").value(UserHttpResponseCode.TOKEN_INVALID.getCode()))
        .andExpect(jsonPath("$.message").value(UserHttpResponseCode.TOKEN_INVALID.getClientMessage()));
  }
}
