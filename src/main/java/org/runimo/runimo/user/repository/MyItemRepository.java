...
public interface MyItemRepository extends JpaRepository<UserItem, Integer> {
    ...
    @Query(
        "SELECT new org.runimo.runimo.user.service.dto.InventoryItem(i.itemId, it.name, i.quantity, it.imgUrl) "
            +
            "FROM UserItem i" +
    ...
    @Query(
        "SELECT new org.runimo.runimo.user.service.dto.InventoryItem(it.itemId, e.name, it.quantity, e.imgUrl) "
            +
            "FROM UserItem it" +
