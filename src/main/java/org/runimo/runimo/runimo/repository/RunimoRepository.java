...
public interface RunimoRepository extends JpaRepository<Runimo, Long> {
    ...
    @Query("""
            select new org.runimo.runimo.runimo.service.dto.RunimoSimpleModel(r.id, rd.code, r.totalRunCount, r.totalDistanceInMeters)
            from Runimo r
    ...
    @Query("""
            select new org.runimo.runimo.runimo.service.dto.MainRunimoStat(rd.name, rd.imgUrl, r.totalRunCount, r.totalDistanceInMeters)
                  from Runimo r
                  join RunimoDefinition rd on rd.id = r.runimoDefinitionId
                  where r.id = :runimoId
        """)
